...
{
    intr_count++;
    //current clock cycle + 100 ms * 32.678 KHz = Hz
    //from one rising edge to the other rising edge
    set_cycles(get_cycles() + 100* 32.678);
}
...
{
    //interrupt has to be set to 1 to enable
    write_csr(mstatus, read_csr(mstatus) | (1 << MSTATUS_MIE_BIT));
}

void disable_interrupt()
{
    //interrupt has to be set to 0 to disable
    write_csr(mstatus, read_csr(mstatus) & ~(1 << MSTATUS_MIE_BIT));
}
